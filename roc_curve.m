...
function ROC_data = roc_curve(class_1, class_2, dispp, dispt)
    ...
    elseif(dispt == 2)
        fprintf('\n ROC CURVE PARAMETERS\n');
        fprintf(' Distance\t\tThreshold\t\tSensitivity\t\tSpecificity\t\tAROC\t\tAccuracy\t\tMK\t\t\tBM\t\t\tMCC\t\tF1 score\t\tFOR\t\tFDR\t\tFPR\t\tFNR\t\tNPV\t\tPPV\t\t\n');
        fprintf('%.4f\t\t\t%.4f\t\t\t%.4f\t\t\t%.4f\t\t\t%.4f\t\t%.4f\t\t\t%.4f\t\t%.4f\t\t%.4f\t%.4f\t\t%.4f\t\t\t%.4f\t\t\t%.4f\t\t\t%.4f', distance(opt), param.Threshold, param.Sensi,param.Speci, ...
            param.AROC,param.Accuracy, param.PPV, param.NPV, param.FNR, param.FPR, param.FDR, ...
            param.FOR, param.F1_score,param.MCC, param.BM, param.MK);
        fprintf(' \n');
    end
