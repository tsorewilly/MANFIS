...
%KEYWORDS: perf, performance, metric, performance measure, machine learning
%}
%% Comments
%{
Abbreviations:
a: absolute; e: error(s); M: METRIC; m: mean; P: PREDICTIONS; p: percentage
r: relative (if before e); r: square root (if before m); s: squared; T: TARGETS
V: VALUE(S);
Calculation tree:
e
|
...
Inputs=inputParser;
Inputs.addRequired('T',@(x) isnumeric(x) && ndims(x)==2 && (size(x,1)==1 || size(x,2)==1));
Inputs.addRequired('P',@(x) isnumeric(x) && ndims(x)==2 && (size(x,1)==1 || size(x,2)==1));
